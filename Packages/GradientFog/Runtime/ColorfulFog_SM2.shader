...
Shader "Hidden/JG/ColorfulFog"
{
	...
	// ADDED BY NOTHKE
	/*
	float hash(half3 p)  // replace this by something better
	{
	...
	float noise(half3 x)
	{
		...
		return lerp(lerp(lerp(hash(p + half3(0, 0, 0)),
			hash(p + half3(1, 0, 0)), f.x),
			lerp(hash(p + half3(0, 1, 0)),
				hash(p + half3(1, 1, 0)), f.x), f.y),
			lerp(lerp(hash(p + half3(0, 0, 1)),
				hash(p + half3(1, 0, 1)), f.x),
				lerp(hash(p + half3(0, 1, 1)),
					hash(p + half3(1, 1, 1)), f.x), f.y), f.z);
	}*/
	...
	half4 ComputeFog(v2f i, bool distance, bool height, int colorMode) : SV_Target
	{
		...
		half3 t = half3(_Time.x, 0, 0);

		half3 eval = wsPos;

		half nois =
			//noise(twsPos * 16) * 0.2 +
			noise((eval + t * 32 * 5) * 0.2) * 0.2 +
			noise((eval + t * 16 * 5) * 0.1) * 0.2 +
			noise((eval + t * 8 * 5) * 0.05) * 0.2 +
			noise((eval + t * 4 * 5) * 0.025) * 0.2 +
			noise((eval + t * 2 * 5) * 0.01) * 0.2;*/

		//half4 fin = lerp(fogColor, sceneColor, fogFac);
