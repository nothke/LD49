...
float2 randcirc(float2 uv)
{
	float t = nrand(uv.xy) * 2 * PI;
	float r = nrand(uv.yx);
	return float2(cos(t), sin(t)) * r;
}

inline half3 PerPixelNormal(sampler2D bumpMap, half4 coords, half3 vertexNormal, half bumpStrength) 
{
	half3 bump = (
		UnpackNormal(tex2D(bumpMap, coords.xy)) + 
		UnpackNormal(tex2D(bumpMap, coords.zw))) * 0.5;
